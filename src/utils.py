...
def similarity(embedded, w, b, N=config.N, M=config.M, P=config.nb_proj, center=None):
    ...
    if center is None:
        center = tf.reduce_mean(embedded_split, axis=1) #[N, P]
        center_except = tf.reshape(tf.reduce_sum(embedded_split, axis=1, keep_dims=True) - embedded_split, shape=[N * M, P]) / (M - 1) #[NM, P]
        ...
        S = tf.concat([
            tf.concat([cossim(center[i, :], embedded_split[j, :, :])
